//0 * * * * 鐐稿勾鍏� 鑷姩鍏ヤ細銆佺鍒般€佷换鍔°€佸崌绾с€佸紑瀹濈銆佹崱閲戝竵
//鍗婃畫鍝侀殢渚胯窇璺� 
// 瀛︿範浜ゆ祦 
const $ = new Env('鎼炲熀澶х-鐐稿勾鍏�');
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            for (let i = 0; i < $.newShareCodes.length && true; ++i) {
                console.log(`\n寮€濮嬪姪鍔� 銆�${$.newShareCodes[i]}銆慲)
                let res = await getInfo($.newShareCodes[i])
                if (res && res['data'] && res['data']['bizCode'] === 0) {
                    if (res['data']['result']['toasts'] && res['data']['result']['toasts'][0] && res['data']['result']['toasts'][0]['status'] === '3') {
                        console.log(`鍔╁姏娆℃暟宸茶€楀敖锛岃烦鍑篳)
                        break
                    }
                    if (res['data']['result']['toasts'] && res['data']['result']['toasts'][0]) {
                        console.log(`鍔╁姏 銆�${$.newShareCodes[i]}銆�:${res.data.result.toasts[0].msg}`)
                    }
                }
...
function transform(str) {
    ...
    for (i = 0; i < aParams.length; i++) {銆€銆€
        var aParam = aParams[i].split("=");銆€銆€
                    ...
                    if (safeGet(data)) {
                        ...
                        if (data.code === 0) {
                            if (data.data && data['data']['bizCode'] === 0) {
                                ...
                                console.log(`\n\n 鎴愬姛棰嗗彇${data.data.result.produceScore}涓竵`)
                            }
                        } else {
                            //console.log(`\n\nsecretp澶辫触:${JSON.stringify(data)}\n`)
                        }
                            ...
                            if (data.data && data['data']['bizCode'] === 0) {
                                for (let i = 0; i < data.data.result.myAwardVos.length; i++) {
                                    switch (data.data.result.myAwardVos[i].type) {
                                        case 15:
                                            console.log(`\n\n 鑾峰緱${data.data.result.myAwardVos[i].pointVo.score}甯乣)
                                            break
                                        case 1:
                                            //console.log(`\n\n 鑾峰緱浼樻儬鍒� 婊�${data.result.myAwardVos[1].couponVo.usageThreshold}-${data.result.myAwardVos[i].couponVo.quota}  ${data.result.myAwardVos[i].couponVo.useRange}`)
                                            break
                                    }
                                }
                            }
